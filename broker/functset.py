...
def get_graph_attribute(graph):
    path=list(nx.all_pairs_dijkstra_path(G)) # 生成每对节点之间的最短路径
    ...
    length=list(nx.all_pairs_dijkstra_path_length(G)) # 生成每对节点之间的最短距离
    ...
    return np.array(dis_list), update_path_list
